...
- **Employer-panel placement TBD** — David has 3 options to choose from (see Next steps §3). Recommendation: collapsible "What this score means →" toggle (option 2).
...
3. **Decide employer-panel placement** with David before building. Three options:
   - (1) Permanent inside hero device — every visitor sees it. Cost: hero is tall.
   - (2) Collapsible "What this score means →" toggle — recommended.
   - (3) Score widget on hero links to `/score` deep-link page where the panel lives — cleanest hero, deepest copy lives where employers click.
4. **Build `<ScoreExplainer>` component** at `src/components/marketing/score-explainer/score-explainer.tsx`. 5 bullets, copy locked from mockup lines 155-185. Numbers via `import { TOTAL_MANDATORY_LESSONS, TOTAL_OPTIONAL_LESSONS, TOTAL_COURSE_MONTHS } from "@/lib/config"`. Add Vitest covering all 5 bullets render with correct numbers.
5. **Update `HeroDevice`** at `src/components/marketing/hero/hero-device.tsx` to (a) render the GWTH icon header inside the score card (use `<Image src="/icon.png" alt="GWTH" width={36} height={36} />`), (b) include `<ScoreExplainer>` per the placement decision in §3.
