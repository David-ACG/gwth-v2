...
const providers: { id: OAuthProvider; label: string; icon: React.ReactNode }[] = [
  ...
  },
  {
    ...
    label: "LinkedIn",
    icon: (
      <svg viewBox="0 0 24 24" className="fill-[#0A66C2]" aria-hidden="true">
    ...
    <div className={styles.oauthGroup}>
      {providers.map(({ id, label, icon }) => (
        <button
          key={id}
          type="button"
          className={`${styles.buttonOutline} ${styles.oauthButton}`}
...
/**
 * Hairline divider with a centered mono label, used between the OAuth buttons
 * and the email form.
 */
