...
/**
 * Colour options for live exploration on the redesign_v2 page.
 *
 * Three axes:
 * - Light postscript panel (the bottom green/sage in light mode)
 * - Dark postscript panel (the bottom green/forest in dark mode)
 * - Primary red CTA (Get Started button + score widget + accents)
 *
 * Each swatch overrides the corresponding CSS variable on the
 * `[data-variant="e2-e"]` wrapper via inline style. The wrapper still
 * inherits everything else from the E2-E token block in globals.css.
 */

type Swatch = { hex: string; label: string }

const LIGHT_PANEL_OPTIONS: readonly Swatch[] = [
  { hex: "#6f876c", label: "Original sage" },
  { hex: "#7d8770", label: "Warm sage" },
  { hex: "#8a9075", label: "Olive sage" },
  { hex: "#7d7e5e", label: "Warm olive" },
  ...
  { hex: "#a89376", label: "Warm sand" },
] as const

const DARK_PANEL_OPTIONS: readonly Swatch[] = [
  { hex: "#2f442c", label: "Dark forest (E2-F)" },
  { hex: "#243d22", label: "Deeper forest" },
  { hex: "#3a5234", label: "Lighter forest" },
  { hex: "#1f3a1d", label: "Very dark forest" },
  { hex: "#3e5238", label: "Medium sage" },
  { hex: "#2c3e2a", label: "Mid forest" },
  { hex: "#385141", label: "Sage-teal" },
  { hex: "#28392a", label: "Bottle" },
...
const PRIMARY_OPTIONS: readonly Swatch[] = [
  ...
  { hex: "#963a2e", label: "Dusty brick" },
  { hex: "#7e2520", label: "Oxblood" },
  { hex: "#b03a30", label: "Vibrant brick" },
...
/**
 * Live palette explorer for E2-E. Sticky control bar lets a reviewer
 * cycle through 12 options per axis (light panel / dark panel / red CTA)
 * and immediately see the page below render with the chosen colours.
 *
 * The page content is the full E2-E layout copied from
 * `src/app/redesign/v-e-2-e/page.tsx` so colour interactions can be
 * judged in context.
 */
    ...
    <>
      ...
      <div className="sticky top-0 z-50 border-b border-neutral-200 bg-white/95 backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-900/95">
        <div className="mx-auto max-w-6xl space-y-2 px-4 py-3 sm:px-6">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-baseline gap-3">
              ...
              <span className="hidden text-[11px] text-neutral-500 sm:inline">
                E2-E palette explorer · click any swatch to apply
              </span>
            </div>
          ...
          <SwatchRow
            label="Light postscript panel"
            options={LIGHT_PANEL_OPTIONS}
            value={lightPanel}
            onChange={setLightPanel}
          />
          <SwatchRow
            label="Dark postscript panel"
            options={DARK_PANEL_OPTIONS}
            value={darkPanel}
            onChange={setDarkPanel}
          />
          <SwatchRow
            label="Red CTA"
      ...
      {/* ───── E2-E content with live overrides ────────────────────── */}
      <div
        data-variant="e2-e"
      ...
      >
        ...
        <header className="border-b-2 border-foreground bg-background">
          <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-6 px-5 sm:px-8">
            <Link href="/redesign_v2" className="flex items-baseline gap-3">
        ...
        {/* ───── Postscript — driven by --variant-panel-bg ────────── */}
        ...
        <footer className="bg-background">
          <div className="mx-auto max-w-6xl px-5 py-14 sm:px-8">
            ...
            <div className="mt-6 flex flex-col gap-3 text-[11px] uppercase tracking-[0.14em] text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
              ...
              <span>redesign_v2 · E2-E palette explorer</span>
            </div>
          </div>
        </footer>
