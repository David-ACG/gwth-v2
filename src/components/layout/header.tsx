"use client"

import { Search, Menu, LogOut, User } from "lucide-react"
import { Button } from "@/components/ui/button"
import { ThemeToggle } from "@/components/shared/theme-toggle"
import { useSidebar } from "@/hooks/use-sidebar"
import { useSearch } from "@/hooks/use-search"
import { BreadcrumbNav } from "@/components/layout/breadcrumb-nav"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuSeparator,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu"
import { signOut } from "@/lib/actions/auth"
import { cn } from "@/lib/utils"
import styles from "./header-fde.module.css"

interface DashboardHeaderProps {
  /** Authenticated user's display name */
  userName?: string | null
  /** Authenticated user's email */
  userEmail?: string | null
  /** Authenticated user's avatar URL (from OAuth provider) */
  userAvatarUrl?: string | null
}

/**
 * Dashboard header with breadcrumb, search trigger, theme toggle, and user
 * dropdown, in the FDE journal register (DESIGN_FDE.md §5.9): solid paper
 * bar, hairline bottom rule, mono search trigger, square teal avatar.
 */
export function DashboardHeader({ userName, userEmail, userAvatarUrl }: DashboardHeaderProps) {
  const { isMobile, open: openSidebar } = useSidebar()
  const { open: openSearch } = useSearch()

  const initials = userName
    ? userName
        .split(" ")
        .map((n) => n[0])
        .join("")
        .toUpperCase()
        .slice(0, 2)
    : "U"

  return (
    <header
      className={cn(
        styles.shell,
        styles.header,
        "sticky top-0 z-40 flex h-16 items-center gap-4 px-4 md:px-6"
      )}
    >
      {isMobile && (
        <Button
          variant="ghost"
          size="icon"
          onClick={openSidebar}
          aria-label="Open navigation"
          className={cn("size-8", styles.iconButton)}
        >
          <Menu className="size-5" />
        </Button>
      )}

      <BreadcrumbNav />

      <div className="ml-auto flex items-center gap-2">
        {/* Search trigger (desktop): square hairline field, mono label */}
        <button
          type="button"
          onClick={openSearch}
          className={styles.searchTrigger}
        >
          <Search className="size-4" aria-hidden="true" />
          <span>Search</span>
          <kbd className={styles.kbd}>⌘K</kbd>
        </button>
        <Button
          variant="ghost"
          size="icon"
          onClick={openSearch}
          className={cn("size-8 sm:hidden", styles.iconButton)}
          aria-label="Search"
        >
          <Search className="size-4" />
        </Button>

        {/* Theme toggle */}
        <ThemeToggle className={styles.iconButton} />

        {/* User dropdown */}
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className={cn("size-8", styles.avatarButton)}
            >
              <Avatar className={cn("size-8", styles.avatar)}>
                {userAvatarUrl && (
                  <AvatarImage
                    src={userAvatarUrl}
                    alt={userName ?? "User"}
                    className={styles.avatarImage}
                  />
                )}
                <AvatarFallback className={styles.avatarFallback}>
                  {initials}
                </AvatarFallback>
              </Avatar>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent
            align="end"
            className={cn("w-56", styles.shell, styles.menu)}
          >
            <div className="px-2 py-1.5">
              <p className={styles.menuName}>{userName ?? "User"}</p>
              {userEmail && <p className={styles.menuMeta}>{userEmail}</p>}
            </div>
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild>
              <a href="/profile" className="cursor-pointer">
                <User className="mr-2 size-4" />
                Profile
              </a>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
              onClick={() => signOut()}
              className={cn("cursor-pointer", styles.menuDanger)}
            >
              <LogOut className="mr-2 size-4" />
              Sign out
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    </header>
  )
}
