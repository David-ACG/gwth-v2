# The paper-first register (current for new and rebuilt surfaces since 2026-09-02)

> **Where it can be seen.** The N12 build that carries this register is awaiting David's
> review on the hlab preview (https://hlab.taila51191.ts.net:9458). Production gwth.ai
> still runs the older FDE register until N12 is approved and published, so a
> screenshot of gwth.ai is legacy evidence, never a reference for this register.
> Resolve the current Bible items for a surface with
...
| Plates (images) | `public/home/paper/<name>.png` and `<name>-dark.png`, 1376x768. The existing plates were generated in the I2 stripped style, which was superseded on 2026-09-10 by bible `landscape-labelled-images` (two landscape light and dark versions, useful labels in the image, tactile paper, dark made as an edit of the light). They are kept as they are; regenerating them is a separate decision. Never overwrite: archive dated. |
...
- **Images carry the colour.** No frame, no tint overlay, 8px radius. The page title stays page text. Generated illustrations follow bible `landscape-labelled-images` (2026-09-10): teaching labels are embedded in the image, large and legible, checked in both light and dark versions and at phone width. The earlier page-key-only rule (X6) was retired by that decision; pages built with an X6 key keep it until they are next rebuilt.
...
| `/for-teams`, `/pricing`, `/lessons`, `/labs`, `/about`, `/contact`, `/newsletter`, `/why-gwth`, legal, news, tech radar, verify, waitlist | **Bridged**: their `*-fde` modules lost the duplicated palette blocks, the retired faces and the mono kicker treatment mechanically (see the N12 packet); layout unchanged. They read in the new family, while their composition is still the FDE one: where a module says `.masthead { background: var(--v-quiet) }` there used to be a teal band, and its comments may still say so. Rebuild per page as each is touched, and rename `.mono` classes and `--fde-mono-*` sizes when you do. |
...
| Rendered video | Adopts this register (bible `paper-first-video-format`, David 2026-09-07: "yes, fully"; verdict still pending on /bible). Format stays linear. Re-rendering the existing library is a separate decision. |
