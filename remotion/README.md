...
> tree, and is excluded from the main app's `tsc --noEmit` typecheck (see the
> root `tsconfig.json` `exclude`) so the two trees don't collide in CI. The
> slide templates here are the keepable deliverable; the explainer is one
