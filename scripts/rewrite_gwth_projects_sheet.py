...
MONTH_1 = [
    ...
    ("Core build", "1", "L6 + L12",
     ...
     "Lovable site comparing AI course offerings (Coursera, Maven, DeepLearning.AI, GWTH). Includes pricing, duration, level, outcomes.",
     "SEO-friendly; natural GWTH channel; low-maintenance.",
     "External site already exists (aicoursedirectory.com) - check whether to buy or compete.",
     ...
     "S",
     "L6 Research; L12 Website",
     "Promoted Row 51 from unlabelled to M1 core build."),
...
MONTH_2 = [
    ...
    ("Core build", "2", "L13",
     ...
     "(David to choose — gcseready.app? spanishgcse.app?)",
     "(David to choose)",
     "Conversational voice app that role-plays a Pearson Edexcel GCSE Spanish examiner. Asks curriculum-aligned questions; marks vocabulary and fluency.",
     "Solves a real family pain (David's daughter); ElevenLabs or OpenAI Realtime with 300k credits David already has.",
     ...
     "Prototype already built per David's note. Cost model: prefer existing ElevenLabs credits; measure per-hour cost."),
    ...
    ("Core build", "2", "L11",
     ...
     "(David to choose — trainclaude.dev? offlinecode.app?)",
     "(David to choose)",
     ...
     "Kept M2. Niche but technically sound."),
    ...
    ("New — from lessons", "2", "L4",
     "API-Powered Mini-App",
     "(David to choose)",
     "(TBD)",
     "First Next.js app that calls the Claude API directly (not through a chat UI). Student ships a single-purpose tool (summariser, email drafter, idea scorer, etc.) with streaming.",
     "First real 'code' the student ships. Every subsequent lesson builds on this pattern.",
     "Key storage; rate limit handling.",
     "Next.js 16 + Vercel AI SDK v5 + Claude Sonnet 4.6 + Vercel Hobby",
     "GPT-5; Gemini 3 Pro via SDK",
     "Free (demo); Paid if public traffic",
     "Yes",
     "S",
     "L2 Dev Env; L4 APIs",
     "**ADDED** - generic L4 build not currently in sheet. David to assign a domain."),

    ("New — from lessons", "2", "L6",
     "Document Parser -> Structured JSON",
     "(David to choose)",
     "(TBD)",
     "Upload PDF / Word / Excel -> returns clean JSON / Markdown. Uses Docling + Claude. Foundation for every RAG project.",
     "Teaches document-ingestion patterns used in every M2 agent and RAG build.",
     "Scanned PDFs need OCR; tables often mangled - learn the fallbacks.",
     "Docling + Next.js + Supabase Storage",
     "Unstructured.io; LlamaParse; textract",
     "Freemium (free tier, paid = batch + higher limits)",
     "Yes",
     "S",
     "L2 Dev Env; L6 Ingest",
     "**ADDED** - L6 build not in sheet."),

    ("New — from lessons", "2", "L11",
     "MCP Email-Triage Agent",
     "(David to choose — inboxpilot.app? emailzen.app?)",
     "(TBD)",
     "Claude Agent SDK + MCP Gmail connector: agent reads inbox, classifies, drafts replies, flags urgent, runs daily at 7am. Teaches CLIs-first + MCP-second pattern.",
     "Massive personal productivity win; showcases Anthropic's agent SDK + MCP ecosystem.",
     "OAuth permissions; risk of auto-replying without review - start with draft-only mode.",
     "Claude Agent SDK + MCP Gmail + gh CLI / Gmail CLI + Cloudflare Worker (schedule)",
     "n8n with Gmail trigger; Make.com AI Agents; Zapier AI Agents",
     "Free (personal) - Paid if multi-account (SaaS)",
     "Yes",
     "M",
     "L2 Dev Env; L4 APIs; L5 Security; L11 Agents",
     "**ADDED** - L11 core build; central to the CLI+MCP story."),

    ("New — from lessons", "2", "L12",
     "UK Planning Applications Research Agent",
     "(David to choose — planningradar.co.uk?)",
     "(TBD)",
     ...
     "**ADDED** - L12 core build; strong UK positioning."),

    ("New — from lessons", "2", "L15",
     "Production Automation Stack (Stripe + n8n + Supabase)",
     "(David to choose)",
     "(TBD)",
     "Self-hosted n8n 2.0 workflow: Stripe webhook -> Supabase update -> Claude welcome email -> Slack alert. Full production automation pattern.",
     "Every M2 SaaS build reuses this pattern. Teaches self-hosted infra.",
     "n8n 2.0 hosting (UK Hetzner / Coolify); Stripe webhook signing; retry-on-failure.",
     "n8n 2.0 (self-hosted Coolify) + Stripe + Supabase",
     "Make.com; Zapier Enterprise",
     "Paid (template used in every SaaS build)",
     "Yes (internal tooling — not public)",
     "M",
     "L2 Dev Env; L15 Automation",
     "**ADDED** - L15 core build not in sheet."),
]

MONTH_3 = [
    ("Core build", "3", "L10 + M3 Mini-Build 4",
     "Internal AI Skills Library (Dojo Clone)",
     "(David to choose — aiskillshub.com? teamdojo.app? vibeappmarket.com?)",
     "vibeappmarket.com (candidate)",
     ...
     "Operationalises 'Harness Engineering' at SME scale; answer to 'where are all our good prompts?'",
     ...
     "**ADDED** - M3 Mini-Build 4 not in sheet. Candidate domain vibeappmarket.com."),
    ...
    ("Core build", "3", "L14 + L15 + M3 Mini-Build 3",
     ...
     "(David to choose — righttodeploy.app? aigovernancekit.com?)",
     "(TBD)",
     ...
     "Highest ROI enterprise deliverable - compresses compliance days into minutes in regulated UK sectors.",
     ...
     "**ADDED** - M3 Mini-Build 3 not in sheet."),
]
...
DEPRECATED = [
    ...
    ("Placeholder — needs decision", "—", "—",
     ...
     "", "vibeappmarket.com", "", "", "", "", "", "", "", "", "",
     "Row 58 - suggested repurposing for M3 Dojo Clone / Internal Skills Library."),
]
...
M2_LESSONS = [
    ...
    ("2", "L4", "Calling Model APIs", "API-Powered Mini-App", "**API-Powered Mini-App (David to choose)**", "ADDED to sheet"),
    ("2", "L5", "Security — Keys, Injection, GDPR, ICO", "Hardened Project Template", "—", "Template, not public"),
    ("2", "L6", "Ingesting Real Business Data", "Document Parser", "**Document Parser (David to choose)**", "ADDED to sheet"),
    ...
    ("2", "L11", "Agents — Claude SDK, CLIs, MCP", "MCP Email-Triage Agent", "**inboxpilot/emailzen (ADDED)**; Train Coder tool", "ADDED to sheet"),
    ("2", "L12", "Browser & Computer-Use Agents", "Planning Applications Research Agent", "**planningradar (ADDED)**; sharetrajectory.com; stagsguide/hensguide; newsbot.biz; blogbot.app", "ADDED + others"),
    ...
    ("2", "L15", "Production Automation (n8n 2.0)", "Multi-Step Automation (n8n+Stripe+Supabase)", "**Production Automation Stack (ADDED)**; sitegeo.net; newsbot.biz; blogbot.app", "ADDED + others"),
...
M3_LESSONS = [
    ...
    ("3", "L10", "Harness Engineering (Ramp Glass)", "Internal Skills Library (Dojo)", "**vibeappmarket.com / aiskillshub (ADDED)**", "ADDED to sheet"),
    ("3", "L14", "Proprietary Data Advantage", "(strategy)", "—", "—"),
    ("3", "L15", "Right to Deploy", "Right-to-Deploy Template Generator", "**righttodeploy.app / aigovernancekit (ADDED)**", "ADDED to sheet"),
...
out = r"C:\Users\david\Downloads\All GWTH Project Ideas 21_Apr_26 v3.xlsx"
wb.save(out)
print(f"Saved: {out}")
